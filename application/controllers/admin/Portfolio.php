...
class Portfolio extends Admin_Controller {
    ...
    function __construct()
    {
        ...
        define('THIS_URL', base_url('admin/portfolio'));
    ...
    function index()
    {
        ...
        // get list
        $portfolio = $this->portfolio_model->get_all($limit, $offset, $filters, $sort, $dir);

        // build pagination
        $this->pagination->initialize(array(
            'base_url'   => THIS_URL . "?sort={$sort}&dir={$dir}&limit={$limit}{$filter}",
            'total_rows' => $portfolio['total'],
            'per_page'   => $limit
        ));

        // setup page header data
        $this->set_title('Portfolio');
        ...
        // load views
        $data['content'] = $this->load->view('admin/portfolio/list', $content_data, TRUE);
    ...
    {   
        ...
        {   
            ...
            $files = $files['images'];
                ...
                {   
                    ...
                    ->set_output(json_encode(array('result' => 1,'message' =>'New Portfolio  Added','img'=>$upload_image['name'])));
        ...
        // load views
        $data['content'] = $this->load->view('admin/portfolio/form', $content_data, TRUE);
        $this->load->view($this->template, $data);
    ...
    function edit($id=NULL)
    {
        ...
        // get the data
        $user = $this->portfolio_model->get_id($id);
        ...
        if (!empty($postData))
        {
            ...
            if(!empty($files['name'])){
                ...
                {   
                     ...
                     $this->output
                    ->set_content_type('application/json')
                    ->set_output(json_encode(array('result' => 1,'message' =>'Portfolio  Updated')));
            ...
            $saved = $this->portfolio_model->edit($postData,'id');
            if ($saved)
                {   
    ...
    function delete($id=NULL)
    {
        ...
        if ( ! is_null($id) OR ! is_numeric($id))
        {
            ...
            if ($user)
            {
                ...
                $delete = $this->portfolio_model->delete_page($id);

                if ($delete)
                {
                    $this->session->set_flashdata('message', ' Portfolio '  .$user['title'] . ' Deleted');
                }
                else
                {
                    $this->session->set_flashdata('error', 'Can not delete Portfolio '  .$user['title'] . ' ');

                }
            }
            else
            {
                $this->session->set_flashdata('error', 'portfolio error not_exist');
            }
        }
        else
        {
            $this->session->set_flashdata('error', 'portfolio error id_required');
        }
    ...
    {
        if (trim($name) != trim($current) && $this->portfolio_model->name_exists($name))
        {
    ...
    {
        if (trim($email) != trim($current) && $this->portfolio_model->email_exists($email))
        {
            $this->form_validation->set_message('_check_email', sprintf(lang('portfolio error email_exists'), $email));
            return FALSE;
        }
        else
        {
   ...
   private function _do_upload($post_image,$data,$update,$original_name='')
    {
            ...
            $curr_month = date('M_Y');
             ...
             if (!is_dir('uploads/portfolio/'.$curr_month)) {
                 mkdir('./uploads/portfolio/' . $curr_month, 0777, TRUE);
             }
            ...
            $upload['upload_path'] = dirname($_SERVER["SCRIPT_FILENAME"]) .'/uploads/portfolio/'.$curr_month;
          ...
          $file_name = $upload['file_name'] = time() .'_' .date('d').'.'.$file_ext ;
           ...
           if($upload_success){
                ...
                $img_data['path'] ='uploads/portfolio/'.$curr_month.'/'.$file_name;
                ...
                if($update == 1){
                    ...
                    $save = $this->portfolio_model->edit($img_data,'portfolio');


                }else{

                    $save = $this->portfolio_model->add($img_data,'portfolio');
