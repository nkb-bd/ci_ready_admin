...
class Projects_model extends CI_Model {
     ...
     */
    function get_all_for_admin($tbl='',$limit=0, $offset=0, $filters=array(), $sort='id', $dir='ASC')
    {
        ...
        $sql = "
            SELECT SQL_CALC_FOUND_ROWS projects.file_path,details,approved,users.username as u_name, users.id as u_id, projects.title , projects.id as id, budget,category,posted_by, category.name as cat_name,sale_limit,sale_count
            FROM {$tbl}
  ...
  /**
    ...
    function get_project_by_id($id) {
        ...
        $query = $this->db->select('first_name,last_name,users.created as seller_joined,users.profile_img as seller_img, users.id as seller_id,username,projects.id as id,category.id as cat_id,location,projects.created, category.price,sub_category.id as sub_cat_id, posted_by,budget,title,details,projects.id as project_id,projects.file_path,projects.file_name,category.name as cat_name, sub_category.name as sub_cat_name,sale_limit,sale_count,projects.duration');
        $query = $this->db->from('projects');
        $query = $this->db->where('projects.id', $id);
        $query = $this->db->join('users', 'users.id = projects.posted_by');
